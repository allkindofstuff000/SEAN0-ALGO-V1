...
export default function BtcRsiEma() {
  ...
  const sigOpen = btcSignals.length - sigResolved.length;
  ...
  const btcParams = (start: string, end: string) => ({
  ...
  const handleRunTest = () => {
    const bal = balance;
    setWf(null);
    runBacktest.mutate(btcParams(startDate, endDate), {
      onSuccess: (res) => {
        if (res.error) {
            ...
            <div className="rounded-lg border border-border bg-card p-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-4 items-end mt-4">
                <div className="space-y-2">
                  ...
                  <Slider value={lag} onValueChange={setLag} min={0} max={5} step={0.25} />
                </div>
              </div>
