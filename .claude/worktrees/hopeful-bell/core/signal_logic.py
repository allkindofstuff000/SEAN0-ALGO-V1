...
@dataclass
class SignalLogic:
    """Single-strategy XAUUSD signal engine — dual-timeframe (15m trend + 5m entry).

    Filters applied (no new indicators):
      1. Session filter      — OVERLAP (12-16 UTC) + NEW_YORK (16-21 UTC) only
      2. Trend strength      — |EMA50 - EMA200| > trend ATR
      3. Volatility expansion — entry ATR > 1.1 × ATR-20 average
      4. Range avoidance     — last-20-candle range >= 3 × ATR
      5. Weak candle         — candle high-low range >= 0.5 × ATR
    """
    ...
    def evaluate_xau_strategy(
        ...
        # ── Volatility expansion: ATR > 1.1 × ATR-20 average ───────────────────
        ...
        # ── Weak candle: candle range >= 0.5 × ATR ──────────────────────────────
